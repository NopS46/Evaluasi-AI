...
const mainDashboard = {
    ...
    updateStudentList: function() {
        ...
        const selectedKelas = document.getElementById('filterKelas')?.value || 'Semua';

        const filteredStudents = selectedKelas === 'Semua'
            ? students
            : students.filter(s => s.kelas === selectedKelas);

        if (filteredStudents.length === 0) {
            listDiv.innerHTML = '<div style="padding: 20px; text-align: center; color: #666;">Tidak ada siswa di kelas ini.</div>';
            return;
        }
            ...
            html += `
                <div class="student-item">
                    <div>
                        ...
                        <small>Kelas: ${student.kelas || 'Tidak diketahui'}</small><br>
        ...
        updateKelasFilter: function () {
            ...
            filterSelect.innerHTML = '<option value="Semua">Semua Kelas</option>';
            [...kelasSet].sort().forEach(kelas => {
                const option = document.createElement("option");
                option.value = kelas;
                option.textContent = kelas;
                if (kelas === currentVal) option.selected = true;
...
function predictAllScores() {
    if (!students || students.length === 0) {
        console.warn("❗ Data siswa belum tersedia untuk prediksi");
        return;
    }
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        form.addEventListener("submit", async function (e) {
          ...
          const data = {
            ...
            projectComplexity: parseInt(formData.get("projectComplexity")),
            solutionInnovation: parseInt(formData.get("solutionInnovation")),
            implementationQuality: parseInt(formData.get("implementationQuality")),
            debuggingAbility: parseInt(formData.get("debuggingAbility")),
            presentationScore: parseInt(formData.get("presentationScore")),
          ...
          try {
            ...
            // Tambahkan ke array siswa dan update UI
